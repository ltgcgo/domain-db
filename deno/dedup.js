			...
			for (let sourceId of instanceConfig[target]) {
				...
				for await (let line of TextReader.line(sourceFileHandle.readable)) {
					...
					if (line.indexOf("domain:") === 0) {
						ignoreExact.set(line.substring(7), true);
					} else {
						console.debug(`Ignore Exact Match: "${sourceId}", line ${lineNo} ignored.`);
					};
				};
			};
			break;
		...
		default: {
			...
			for (let sourceId of instanceConfig[target]) {
				...
				for await (let line of TextReader.line(sourceFileHandle.readable)) {
					...
					lineNo ++;
					if (line.indexOf("domain:") === 0) {
						let domainSlice = line.substring(7);
						let domainArray = domainSlice.split(".").reverse();
						if (ignoreExact.has(domainSlice)) {
							console.debug(`"${target}": "${sourceId}", line ${lineNo} ignored with exact match.`);
							continue;
						};
